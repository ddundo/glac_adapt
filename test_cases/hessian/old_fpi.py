...
from glac_adapt.meshadapt import reordered_vec
...
def get_form(mesh_seq):
    def form(index, sols):
        ...
        action = mesh_seq.icepack_solver._diagnostic_solver._model.action(
            ...
            thickness=mesh_seq.h,
            surface=mesh_seq.s,
            fluidity=mesh_seq.options.constants.fluidity,
...
def adaptor(mesh_seq, sols):#, inds):
    ...
    metcom = metric_complexity(hmetricx)
    Nvert = 1000
    d = 2
    ...
    if rank == 0:
        ...
        metric = reordered_vec(load_metric, old_mesh)
...
opts = {"timestep": 10.0, "end_time": 3000.0, "chk_idx": chk_idx}
...
else:  # TODO: fix this numbering, works with chk_idx=0
    with CheckpointFile(f"{options.simulation.input}/adapted_{chk_idx-1}.h5", 'r') as afile:
        initial_mesh = afile.load_mesh(f"mesh_{chk_idx-1}")
